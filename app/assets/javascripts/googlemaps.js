LocationGoogleMap = function() {
  ...
  self.timed_input_on_route = function(){
    clearTimeout(timer);
    self.invalid_locations("listing_origin_loc_attributes");
    self.invalid_locations("listing_destination_loc_attributes");
    timer=setTimeout(
      function() {
        self.startRoute();
      }, 1500
    ...
    if (draggable){
      google.maps.event.addListener(map, "click",
        function(event) {
          ...
          geocoder.geocode({"latLng":event.latLng},self.update_source);
        }
      );

      google.maps.event.addListener(marker, "dragend",
        function() {
          geocoder.geocode({"latLng":marker.getPosition()},self.update_source);
        }
      );
    }
  ...
  self.update_source = function(response,status){
  ...
  self.manually_validate = function(formhint) {
  ...
  self.nil_locations = function(_prefix) {
    ...
    self.manually_validate(_prefix);
  }

  // Make validation fail before it has been checked that the
  // address is found
  self.invalid_locations = function(_prefix) {
    if (!_prefix)
      _prefix = prefix;
    var latitude = document.getElementById(_prefix+ "_latitude");
    latitude.value = null;
  ...
  self.update_model_location = function(place,_prefix){
    if (!_prefix)
      _prefix = prefix;
    var address = document.getElementById(_prefix+ "_address");
    var latitude = document.getElementById(_prefix+ "_latitude");
  ...
  // Rideshare
  self.googlemapRouteInit = function(canvas) {
  ...
  self.startRoute = function(latitude, longitude) {
    ...
    if(foo != '' && bar != '') {
      self.calcRoute(foo, bar);
    } else {
      self.removeRoute();
      if (foo == '' && bar == '') {
        self.setMapCenter(latitude, longitude, true);
        map.setZoom(12);
      }
    }
  }

  self.wrongLocationRoute = function(field){
  ...
  self.wipeFieldsRoute = function(field) {
    document.getElementById(field+"_loc_attributes_address").value = null;
    document.getElementById(field+"_loc_attributes_google_address").value = null;
    document.getElementById(field+"_loc_attributes_latitude").value = null;
    document.getElementById(field+"_loc_attributes_longitude").value = null;
  ...
  self.showRoute = function(orig, dest) {
    var start = orig;
    var end = dest;
  ...
  self.route_not_found = function(orig, dest) {
    if (orig) {
      geocoder.geocode( { 'address': orig}, function(response, status){
         if (!(status == google.maps.GeocoderStatus.OK)) {
          self.nil_locations("listing_origin_loc_attributes");
           self.removeRoute();
         } else {
          self.update_model_location(response, "listing_origin_loc_attributes");
        }
      });
    } else {
      self.nil_locations("listing_origin_loc_attributes");
    ...
    if (dest) {
      geocoder.geocode( { 'address': dest}, function(responce,status){
         if (!(status == google.maps.GeocoderStatus.OK)) {
          self.nil_locations("listing_destination_loc_attributes");
           self.removeRoute();
         } else {
          self.update_model_location(responce, "listing_destination_loc_attributes");
          self.calcRoute(foo, bar);
        }
      });
    } else {
  ...
  self.calcRoute = function(orig, dest) {
    var start = orig;
    var end = dest;
    ...
    if(!orig.match(dest)){
      ...
      directionsService.route(request, function(response, status) {
        if (status == google.maps.DirectionsStatus.OK) {
           directionsDisplay.setDirections(response);
          self.updateEditTextBoxes();
        } else {
          self.removeRoute();
          self.route_not_found(orig,dest);
        }
      });
    } else {
      self.removeRoute();
  ...
  self.updateEditTextBoxes = function() {
    ...
    self.manually_validate("listing_destination");
    self.manually_validate("listing_origin");
  }

  self.addCommunityMarkers = function() {
  ...
  self.initialize_listing_map = function(listings, community_location_lat, community_location_lon, viewport, locale_to_use, use_community_location_as_default) {
    ...
    self.addListingMarkers(listings, viewport);
  }

  self.setMapCenter = function(communityLat, communityLng, preferCommunityLocation) {
  ...
  self.addListingMarkers = function(listings, viewport) {
    // Test requesting location data
    // Now the request_path needs to also have a query string with the wanted parameters
